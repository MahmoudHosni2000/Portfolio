...
export default function Navigation() {
  ...
  const angleTheta = 360 / BtnList.length;
  const container = {
    hidden: {
      opacity: 0,
    },
  ...
  return (
    <div className="w-full fixed h-screen flex items-center xs:justify-center justify-between ">
      <ResponsiveComponent>
      {({ size }) => {
          return size && size >= 480 ? (
            ...
            >
              {BtnList.map((btn, index) => {
                const angleRad = (index * angleTheta * Math.PI) / 180;
